...
| `ProcessingJob` | objetivo/task graph de processamento | processing | job state | ADR-0012, 0016 | não implementado |
...
| `RecognitionArtifact` | texto/layout observado | processing | domain UML | ADR-0012, 0016 | não implementado |
| `ExtractionResult` | campos/tabelas sugeridos | processing | domain UML | ADR-0012, 0016 | não implementado |
| `ClassificationResult` | tipo/rótulo sugerido | processing | domain UML | ADR-0012, 0016 | não implementado |
| `ValidationResult` | regra, resultado e conflito | validation | process sequence | ADR-0013, 0016 | não implementado |
| `RoutingDecision` | próximo passo explicado por política | processing | activity UML | ADR-0016 (proposto) | não implementado |
| `ReviewCase` | unidade de trabalho de revisão | review | process sequence | ADR-0013, 0015 | não implementado |
| `ReviewDecision` | ação humana imutável | review | domain UML | ADR-0013, 0015 | não implementado |
...
| `AuditEvent` | fato/decisão durável | audit | domain UML | ADR-0013, 0015, 0016 | não implementado |
| usuário/permissão | identidade e autorização | auth | ainda não definido | ADR-0015 (gate) | não implementado |
| `FinancialEntry` | lançamento futuro | finance | ainda não definido | ADR-0009, 0013 | não implementado |
| `PaymentIntent` | intenção futura de pagamento | finance | ainda não definido | ADR-0009, 0013, 0015 | não implementado |
